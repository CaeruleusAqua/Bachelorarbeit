...
\subsection{Mosfets}
Als geeignete Mosfets wurden die FDD6690A von Fairchild Semiconductor ausgewählt. Dies sind zugelassen bis zu einer Drain-Source-Spannung von 30V und einer Maximalen dauerbelastung
von 46A, eine gute Kühlung vorausgesetzt. Desweiteren verfügen sie über einen sehr niedrigen Drain-Source-Widerstand von $R_{\text{DS(ON)}}= 12 m$, was zu einer niedrigen Verlustleistung führt. Durch ihre niedrige 
Gate Ladung von 13nC eigen sie sich gut für hohe Schaltfrequenzen. Auf Grund dieser Eigenschaften sind sie Hervorragend für einen Motortreiber geeignet.
...
Durch diese Entscheidung werden Betriebsmodi, welche ein Gleichschalten der Mosfets erfordern erschwert und könn nur duch ein Software PWM realisiert werden. Der Vorteil es Betriebsmodus ``Fast decay, diode recirculation''
ist das es möglich ist Bremsenergie des Fahrzeugs zurück in den Akku zu speisen. Da die Akkus jedoch mit einer Integrierten Ladeelektronik ausgestattet sind, liegt die vermutung nahe, dass ein Laden des Akkus so nicht möglich ist.
Darum wird der Betriebsmodus im Rahmen dieser Arbeit nicht untersucht.

Alle weiteren Anschlüsse des Allegros werden direkt an pins von PORTA am Microcontroller angeschlossen. Da die Fehlerausgänge als Open-Collector aus geführt sind, werden die Internen Pullup-Widerstände des Microcontrollers genutzt.

\section{Motorstrommessung am Shunt}

Da innerhalb ders Wettbewerbes Energieeffizienz wichtig ist, ist es notwendig den aktuellen Stromverbrauch zu kennen. Es wird davon ausgegangen das der Stromverbrauch der Recheneinheit und Sensoren
...
Informationen ist es möglich verschiedenne Fahrregler Auslegungen im Aspekt des Verbrauches zu Vergleichen. Um den Verbrauch zu bestimmen ist es notwendig die anliegende Spannung und des Stom durch den
...
Da der Innenwiederstand des ADC sehr groß ist (\SI{100}{\mohm}) kann der Spannungsteiler als unbelastet betrachtet werden. Damit ergibt sich das Teilungsverhältnis zu $\frac{R_{STL}}{R_{STH}+R_{STL}}=0,153$.
Die maximal messbare Akkuspannung beträgt darurch $\frac{\SI{5}{\V}}{0,152}=\SI{32,7}{\V}$. Die Messung des Stromes gestaltet sich wesentlich aufwändiger und wird im folgenen erläutert.
...
Die Messung des Motorstromes ist mit einem Problem behaftet, da der Motor über eine Pulsweitenmodulation angesteuert wird. Der verlauf des Stomes ist pulsierend, abhängig von
der Frequenz der Pulsweitenmodulation. In \cref{fig:pwm+i_0} zu erkennen ist der Verlauf des PWM Signals (obere Kurve) und des Stomes, welcher nach einer $1-e^t$ Funktion ansteigt.
Ohne weitere Filterung des Signals währe eine extrem hohe abtastfequent des ADC nötig um den genauen Stromfluss zumessen. Da für eine Verbrachsmessung 
der Mittelwert des Stomes interessiert ist, kann dieser mit hilfe eines Tiefpasses erzeugt werden.
...
Ein kostengünstige Variante den Strom zu messen ist den Spannungsabfall über einen Shuntwiderstand zu bestimmen. Die Größe des Shuntwiderstandes sollte so klein wienötig gewählt werden.
Ein zu kleiner Shuntwiderstand benötigt einen starken Messverstärker um das Signal auswerten zu können. Die Quaität und Genauigkeit dieses Signals nimmt allerdings mit steigender Verstärkung ab.
...
Der Shunt wird direkt unter der H-Brücke des Motortreibers gegen Ground geschaltet. So kann wird eine Messung gegen Ground durchgeführt. Die so über den Shuntwiederstand gemessene Spannung könnte 
...
Dieser gibt uns die Möglichkeit des Messignal zu verstärken und gleichzeitig zu
filtern. Da das Signal im optimalen Fall eine Gleichspannung darstellt müssen die Hochfrequenten Anteile des Signales herausfiltern, dies geschieht 
...
Wie im vorherigen Abschnitt diskutiert wird hier ein Sallen-Key Tiefpass entwurfen. Zum Entwurf der Schaltung [\ref{fig:fschalt}] wurde Eagle genutzt.
...
Dabei sind immernur die positiven, reellen Lösungen zu verwenden.


\subsubsection{Bestimmung der Bauteilwerte}

Um die Übersich zu wahren wird die Berechnung der Bauteilwerte hier nicht aufgeführt. Zur Erinnerung
...
In Staubigen oder Nebligen Umgebungen wird das Lichtmöglicherweise zu stark gestreut so das eine Korrekte Messung nicht möglich ist.
...
Nach der Entstörung mit einem 82nF Kondensator direkt am Sensor zwiechen VCC und GND sind die Störungen bereits stark vermindert.
...
Das Messignal vom Sensor wird über einen ADC-Eingang des AVR Microcontrollers ausgelesen. Angeschlossen werden die Sensoren über die PH-Connector Reihe des Herstellers JST, welche auch an den Sensoren verbaut sind.
...
Duch die feste Übersetzung des Getriebes bietet die Messung der Motordehzahl an. Damit lasst sich eine gute Nährung für die aktuelle Geschwindigkeit erreichen. 

\subsection{Hall-Sensor}
Eine Moglichkeit die Motordrehzahl zu messen ist es das Magnetfeld des
...
Alternativ zum Vorgehen mit einem Hall-Sensor wurde es eine andere Lösung Implementiert. An eine Speziell für diesen Motor angefertigte Achsverlängerung wurde eine Inkrementalgeberscheibe befestigt, zusehen in \cref{fig:gabellichtschranke}. 
Diese wird durch einen Sharp GP1A30R Sensor ausgewertet. Dieser liefert ein Drehszahlsignal an den externen Takteingang des Timer 1 vom AVR Microcontroller.
\begin{figure}[H]
\centering
...
der zurück gelegte Weg berechnet werden. Das Übersetzungsverhältnis des Fahrzeugs ist abhängig von 3 Komponenten, dem Stirnrad und dem Motorritzel sowie einer festen internen Übersetzung. Das Verwendtete Stirnrad hat
...
Laut diesem hat eine Servomotor keinen konstanten Stromverbrauch. Der Stomfluss wird immer wieder unterbrochen, so das es zu einem Intervallartigen Stromfluss kommt.
Nur wenn der Servomotor dauerhaft belastet wird kommt es zu einem konstanten Stromfluss.
Im Artikel werden mehrere Servomotorn vermessen, der Motor der dem Verwendeten am nächsten kommt ist der ``Graupner 4421'' mit folgenen Daten:
...
Da der Stromverbrauch der restlichen Komponenten minimal ist werden hier pauschal 100mA veranschalgt.
...
Die Beschaltung erfolgt dabei nach dem Empfehlungen des Datenblattes. Die Verwendten \SI{180}{\uF} Kondensatoren sollen laut Datenblatt low ESR Kondensatoren sein.
Die Verwendten Kondensatoren stammen aus Nichicons L8 Serie und haben einen Serieniderstand von nur \SI{12}{\mohm}.
...
Die Software fungiert aud dem Controller als Service und wartet legendlich auf eine Anfrage von der Seriellen Schnitstelle, welche sie bearbeitet und bei Bedarf beantwortet. 
...
Dies ist Notwendig um bei Asynchronitäten die Warscheinlichkeit eines zufälligen Startsignals zu verringern. Wurde die Preamble erfolgreich empfangen erwartet der Microcontroller eine gültige Topic ID.
...
wenn die vom Client berechnete Checksumme mit der Mitgesendeten identisch ist. Dies ist besonders bei den Daten der Interialsensorik von nöten, da hier defekte Daten zu einem dauerhaften Fehler führen. 
...
Der Ablauf eines schriebvorganges auf dem Microcontroller ist ähnlich. Der Microcontroller wartet auf das Startsignal und eine Preamble sowie eine Topic ID.
Nach dem empfang der TopicID erwartet er die nötigen Daten, sammt Checksumme. Diese ist nötig um die Daten zu verifizieren. Bei der Ansteuerung des Motors kann ein
...
Haufige oder dauerhafte Fehlübertragungen würden duch das fortlaufende Lesen und Schreiben auf dem Microcontroller so oder so zu Fehlern in beiden Richtungen führen.
